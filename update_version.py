...
try:
    with open(manifest, 'r', encoding='utf-8') as f:
        m = f.read()
    m = re.sub(r'android:versionName=".*?"', 'android:versionName="1.0.2"', m)
    m = re.sub(r'android:versionCode=".*?"', 'android:versionCode="10002"', m)
    ...
    if os.path.exists(gradle):
        with open(gradle, 'r', encoding='utf-8') as f:
            g = f.read()
        g = re.sub(r'versionName\s+[\"''\'].*?[\"''\']', 'versionName "1.0.2"', g)
        g = re.sub(r'versionCode\s+\d+', 'versionCode 10002', g)
